...
export async function acceptAdminRoleCommand(
  options: {
    programId: string;
    mint: string;
    authority: string;
  ...
  const cmdLogger = createChildLogger(logger, { command: 'router.accept-admin-role' });
  try {
    ...
    const parsed = validateArgs(RouterAcceptAdminRoleArgsSchema, args);
    if (!parsed.success) {
