...
/**
 * Shared set rate limit admin implementation
 */
export async function setRateLimitAdmin(
  options: SetRateLimitAdminOptions,
  command: CommandContext,
  programType: ProgramName
): Promise<void> {
  const cmdLogger = createChildLogger(logger, { command: `${programType}.set-rate-limit-admin` });
  ...
  try {
    ...
    const parsed = validateArgs(SetRateLimitAdminArgsSchema, {
      programId: options.programId,
      mint: options.mint,
      authority: options.authority,
      newRateLimitAdmin: options.newRateLimitAdmin,
    ...
    if (!parsed.success) {
      ...
      console.error('   • New Rate Limit Admin: Base58 public key (44 characters)');
      console.error('   • RPC URL: Valid HTTP/HTTPS URL');
      process.exit(1);
    }

    const { programId, mint, authority, newRateLimitAdmin, rpcUrl } = parsed.data;
    ...
    console.log('🔄 Generating setRateLimitAdmin transaction...');
    console.log(`   RPC URL: ${rpc}`);
    console.log(`   Program ID: ${programId.toString()}`);
    console.log(`   Mint: ${mint.toString()}`);
    console.log(`   Authority: ${authority.toString()}`);
    console.log(`   New Rate Limit Admin: ${newRateLimitAdmin.toString()}`);
    ...
    if (programType === 'burnmint-token-pool') {
      ...
      ix = await builder.setRateLimitAdmin(mint, authority, newRateLimitAdmin);
    ...
    cmdLogger.info(
      ...
      'setRateLimitAdmin command completed successfully'
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    cmdLogger.error({ error: message }, 'setRateLimitAdmin failed');
