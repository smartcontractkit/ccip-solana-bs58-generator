...
/**
 * Initialize chain remote config command implementation
 */
export async function initChainRemoteConfigCommand(
  ...
  const commandLogger = createChildLogger(logger, { command: 'init-chain-remote-config' });
  ...
  try {
    ...
    commandLogger.error({ error: errorMessage }, 'initChainRemoteConfig command failed');
