...
/**
 * Edit chain remote config command implementation
 */
export async function editChainRemoteConfigCommand(
  ...
  const commandLogger = createChildLogger(logger, { command: 'edit-chain-remote-config' });
  ...
  try {
    ...
    commandLogger.error({ error: errorMessage }, 'editChainRemoteConfig command failed');
