...
export async function transferOwnershipCommand(
  options: { programId: string; mint: string; authority: string; proposedOwner: string },
  command: {
    parent?: {
  ...
  const commandLogger = createChildLogger(logger, { command: 'transfer-ownership' });
  ...
  try {
    ...
    commandLogger.info(
      ...
      'Starting transferOwnership command'
    );
    ...
    const validation = validateArgs(TransferOwnershipArgsSchema, args);
    ...
    console.log(`   Proposed Owner: ${validatedArgs.proposedOwner.toString()}`);

    // Build the instruction using Anchor (async now!)
    console.log('⚙️  Building transaction instruction...');
    const instruction = await instructionBuilder.transferOwnership(
      validatedArgs.mint,
      validatedArgs.authority,
      validatedArgs.proposedOwner
    );
    ...
    TransactionDisplay.displayResults(transaction, 'transferOwnership');
    ...
    commandLogger.error({ error: errorMessage }, 'transferOwnership command failed');
