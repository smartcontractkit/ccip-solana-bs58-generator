...
/**
 * Remove from allow list command implementation
 */
export async function removeFromAllowListCommand(
  ...
  const commandLogger = createChildLogger(logger, { command: 'remove-from-allow-list' });
  ...
  try {
    ...
    commandLogger.info(
      ...
      'Starting removeFromAllowList command'
    );
    ...
    const validation = validateArgs(RemoveFromAllowListArgsSchema, args);
    if (!validation.success) {
      ...
      console.error('');
      console.error(
        'Example remove addresses: ["11111111111111111111111111111112", "22222222222222222222222222222223"]'
      );
      process.exit(1);
    }
    ...
    console.log(`   Remove addresses: ${validatedArgs.remove.length} addresses`);

    // Build the instruction using Anchor (async now!)
    console.log('⚙️  Building transaction instruction...');
    const instruction = await instructionBuilder.removeFromAllowList(
      validatedArgs.mint,
      validatedArgs.authority,
      validatedArgs.remove
    );
    ...
    TransactionDisplay.displayResults(transaction, 'removeFromAllowList');
    ...
    if (errorMessage.includes('Non-base58 character')) {
      ...
      console.error('   • Mint: Base58 public key (44 characters)');
      console.error('   • Authority: Base58 public key (44 characters)');
      console.error('   • Remove addresses: Array of Base58 public keys');
      console.error('   Example: 11111111111111111111111111111112');
    } else if (errorMessage.includes('Invalid JSON')) {
      console.error('❌ Invalid JSON format');
      console.error('💡 Expected format:');
      console.error(
        '   • Remove addresses: ["11111111111111111111111111111112", "22222222222222222222222222222223"]'
      );
